...
public class PatientDAOlmp implements PatientDAO {
    public static String DriverName ="org.sqlite.JDBC";
    public static String url = "jdbc:sqlite:D:/FINAL_OOP_359411110005/Hospital.sqlite";
    public static Connection conn = null;

    public static final String GET_ALL_PAT = "Select * from patient";
    public static final String ADD_NEW_PAT = "Insert Into patient(p_id,p_gender,p_age,p_blood_result)values(?,?,?,?,?,?)";
    public static final String FIND_PAT_BY_ID ="Select* from patient Where p_id=?";
    public static final String UPDATE_PAT ="update patient set p_name=?,p_age=?,p_address=?,p_blood_result=? Where p_id";
    public static final String DELETE_PAT = "delete from patient where p_id=?";

    private static PatientDAOlmp instance =new PatientDAOlmp();
    public  static PatientDAOlmp getInstance() {
        return  instance;

    }
private  PatientDAOlmp(){
        try {
    ...
    public List<Patient> getAllPAT() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String p_addess = rs.getString(5);
                String p_blood_result = rs.getString(6);

                PAT.add(new Patient(id, p_name, p_gender, p_age, p_addess, p_blood_result));
...
try {
    ...
    if (rs != 0) {
        ...
        System.out.println("Could not delte Patient " + "with id" + id);
